...
const Negotiation = () => {
    ...
    const [messages, setMessages] = useState([
        { id: 1, sender: 'supplier', text: 'Initial offer for 5,000 units at ₹12.50 per unit. Delivery in 3 weeks.', time: '2025-01-15, 09:23', details: { qty: '5,000', price: '₹12.50', lead: '21 days' } },
        { id: 2, sender: 'buyer', text: 'Can you do 5,000 units at ₹11.75? Also need delivery in 2 weeks if possible.', time: '2025-01-15, 10:15', details: { qty: '5,000', price: '₹11.75', lead: '14 days' } },
        { id: 3, sender: 'supplier', text: 'We can meet you at ₹12.00 per unit with 2-week delivery. This is our best offer for this quantity.', time: '2025-01-15, 11:42', details: { qty: '5,000', price: '₹12.00', lead: '14 days' } },
    ]);
        ...
        <div className="fade-in" style={{ minHeight: '100vh', background: '#f8fafc' }}>
            ...
            <main className="container" style={{ padding: '3rem 1rem', maxWidth: '1000px' }}>
                <div style={{ marginBottom: '2rem' }}>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>Product: Premium Cotton T-Shirts (SKU: TCT-2025-001)</p>
                </div>
                ...
                <div className="card" style={{ padding: '2rem', minHeight: '600px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2rem', marginBottom: '3rem' }}>
                        {messages.map((msg) => (
                            <div key={msg.id} style={{ display: 'flex', gap: '1rem', flexDirection: msg.sender === 'buyer' ? 'row-reverse' : 'row' }}>
                                <div style={{ width: '40px', height: '40px', background: msg.sender === 'buyer' ? 'black' : '#f1f5f9', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <User size={20} color={msg.sender === 'buyer' ? 'white' : 'black'} />
                                </div>
                                <div style={{ maxWidth: '70%' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                                        <span style={{ fontWeight: '600', color: 'var(--text-main)' }}>{msg.sender === 'buyer' ? 'You' : 'Global Textiles Co.'}</span>
                                        <span>{msg.time}</span>
                                    </div>
                                    <div style={{
                                        background: msg.sender === 'buyer' ? '#1e293b' : '#f8fafc',
                                        color: msg.sender === 'buyer' ? 'white' : 'var(--text-main)',
                                        padding: '1.5rem', borderRadius: '12px', border: msg.sender === 'buyer' ? 'none' : '1px solid var(--border)'
                                    }}>
                                        <p style={{ marginBottom: '1rem', lineHeight: '1.5' }}>{msg.text}</p>
                                        {msg.details && (
                                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                                <span style={{ background: msg.sender === 'buyer' ? 'rgba(255,255,255,0.1)' : '#e2e8f0', padding: '0.25rem 0.75rem', borderRadius: '4px', fontSize: '0.8rem' }}>Qty: {msg.details.qty}</span>
                                                <span style={{ background: msg.sender === 'buyer' ? 'rgba(255,255,255,0.1)' : '#e2e8f0', padding: '0.25rem 0.75rem', borderRadius: '4px', fontSize: '0.8rem' }}>Price: {msg.details.price}</span>
                                                <span style={{ background: msg.sender === 'buyer' ? 'rgba(255,255,255,0.1)' : '#e2e8f0', padding: '0.25rem 0.75rem', borderRadius: '4px', fontSize: '0.8rem' }}>Lead Time: {msg.details.lead}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                    ...
                    <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem' }}>
                            ...
                            <button
                                onClick={() => navigate('/buyer/offer-summary')}
