...
const RouteSuggestion = () => {
    ...
    const handleSuggest = async () => {
        setIsLoading(true);
        try {
            const response = await fetch('http://localhost:5011/api/Logistics/suggest-route', {
                method: 'POST',
    ...
    const handleConfirm = () => {
        if (selectedRoute) {
            navigate('/logistics/route-summary', { state: { jobData, selectedRoute, suggestedRouteData } });
        }
    };
